...
pub enum Type {
    ...
    Fn(Recursive<(TokenTy!["fn"], TokenTy!['(', Vec<Type>, ')'], Option<(TokenTy!["->"], Type)>)>),
}
...
impl Spanned for Type {
    fn span(&self) -> Span {
        match self {
            ...
            Type::Fn(func) => {
                func.with_inner(|(kw_fn, paren, ret)| {
                    let end = ret
                        .as_ref()
...
pub enum Expr {
    ...
    List(Recursive<TokenTy!['[', Vec<Expr>, ']']>),
...
impl Spanned for Expr {
    fn span(&self) -> Span {
        match self {
            ...
            Expr::BinaryOp(binop) => {
                binop.with_inner(|BinOpExpr { lhs, op: _, rhs }| {
                    Span::merge(lhs.span(), rhs.span())
                })
            },
            // recursive span finder
            Expr::UnaryOp(unop) => {
                unop.with_inner(|UnOpExpr { op, expr }| {
                    Span::merge(op.span(), expr.span())
                })
            },
            Expr::CastAs(cast) => {
            ...
            Expr::Call(call_expr) => {
                call_expr.with_inner(|CallExpr { callee, args }| {
                    Span::merge(callee.span(), args.span())
                })
            }
            Expr::Index(index_expr) => {
                index_expr.with_inner(|IndexExpr { array, index }| {
                    Span::merge(array.span(), index.span())
                })
            }
            ...
            Expr::If(if_ref) => {
                ...
                let last_body = expr
                    .else_branch.as_ref().map(|(_, block)| block)
            ...
            Expr::List(list) => list.get_ref().span(),
